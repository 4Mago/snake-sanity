...
function fetchSecrets() {
  ...
  return _client.default.fetch('*[_id == "secrets.mux"][0]').then(secrets => {
...
function saveSecrets(token, secretKey, enableSignedUrls, signingKeyId, signingKeyPrivate) {
  ...
  return _client.default.createOrReplace(doc).then(() => {
    cache.exists = true;
    cache.secrets = {
      token,
      secretKey,
...
function testSecretsObservable() {
  var dataset = _client.default.clientConfig.dataset;
  return (0, _rxjs.defer)(() => _client.default.observable.request({
    url: "/addons/mux/secrets/".concat(dataset, "/test"),
    withCredentials: true,
    method: 'GET'
  }));
